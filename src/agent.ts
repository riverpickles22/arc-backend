// arc's story tools, and the one SDK client the passes share. The embedded
// chat agent that used to live here is gone (A54): Claude Code with the
// arc-canon skill is the chat, under the same constitution and validator.
// What stays is what the passes call — the read tool, the canon and docs
// write tools, mint_id, canonical YAML — all through the same validation
// discipline as any other contributor: failed validation bounces back to
// the model to fix.
...
// The action wire contract lives in arc-canon-graph (graph/api-types.ts),
 ...
 *  shared by the passes that write the record (capture, the material worker)
 *  so all of them go through the same validated gate. The capability is
 *  checked BEFORE validation, so an out-of-scope write never touches the
 *  disk at all; it defaults to UNRESTRICTED, which is what an unclaimed
 *  pass still holds (work-graph §7, an open gap).
 *
