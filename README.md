...
The canon API, the writing passes and the draft layer for [arc](https://github.com/riverpickles22/arc-core).

It serves a story's canon as a JSON graph and runs the writing passes, every one of which edits the record through the same validator a human edit goes through. It exposes no chat: a Claude Code session with the `arc-canon` skill is how an author talks to a story.
...
|---|---|---|
| `ANTHROPIC_API_KEY` | — | Optional. Only the SDK engine reads it; local development does not need one |
...
| `ARC_MODEL` | `claude-opus-5` | Model for the passes |
| `PORT` | `8787` | |
| `ARC_CORS_ORIGIN` | `*` | Set to your frontend origin in production |

## API

**`GET /api/canon`** — the story's canon as one JSON graph (entities, events, relationships, timeline, chapters), generated by arc-core's `export-canon.py`. Cached in memory and invalidated whenever a pass writes canon, so the viewer re-renders on the next fetch.

There is no chat route. A Claude Code session with arc-core's `arc-canon` skill is the chat — same files, same conventions, same validator — so the backend serves the record and the passes, and conversation happens in the terminal the author is already signed in to.

The passes that write through the story tools (capture, draft-scene, the material worker) share that same sandbox:
...
`canonChanged: true` in a response means the client should refetch `/api/canon`.
